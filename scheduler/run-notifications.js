...
function weeksInMonths(m) { return Math.round((m * 365.25) / 12 / 7); }
...
function getCurrentWeekKey(startDateStr, overrides = {}) {
  ...
  for (let i = 0; i < YEAR_IDS.length; i++) {
    ...
    if (now >= yearStart && now < yearEnd) {
      const weekIdx = Math.floor((now - yearStart) / (7*24*60*60*1000));
...
async function sendToUser(db, messaging, uid, title, body, data = {}) {
  ...
  if (DRY_RUN) {
    console.log(`[dry-run] uid=${uid} tokens=${tokens.length} title="${title}"`);
...
async function runTtlyNudge(db, messaging) {
  ...
  for (const doc of snap.docs) {
    const { startDate, weekNotes={}, weekPhotos={}, fcmTokens=[], overrides={} } = doc.data();
    ...
    if ((weekNotes[weekKey] && weekNotes[weekKey].trim()) || weekPhotos[weekKey]) continue;
    const weekNum = getCurrentWeekNumber(startDate);
    sent += await sendToUser(db, messaging, doc.id,
      "How's Week " + weekNum + " going?",
      "Add a note or photo to capture this week before it passes.",
      { url:"/", action:"open_week", weekKey, weekIdx: String(weekIdx) }
    );
  }
  return sent;
}
...
async function runWeeklyRatingNudge(db, messaging) {
  ...
  let sent = 0;
  for (const doc of snap.docs) {
    const { startDate, scores={}, fcmTokens=[], overrides={} } = doc.data();
    if (!startDate || !fcmTokens.length) continue;
    const weekInfo = getCurrentWeekKey(startDate, overrides);
    if (!weekInfo) continue;
    const { weekKey, weekIdx } = weekInfo;
    ...
    const now = new Date(); now.setHours(0,0,0,0);
    if (Math.round((weekEnd - now) / (24*60*60*1000)) > 1) continue;
    if (scores[weekKey] && scores[weekKey] > 0) continue;
    const weekNum = getCurrentWeekNumber(startDate);
    sent += await sendToUser(db, messaging, doc.id,
      "Week " + weekNum + " is ending - how was it?",
      "Rate your week before it slips away.",
      { url:"/", action:"open_rating", weekKey, weekIdx: String(weekIdx) }
    );
  }
  return sent;
}
...
async function runExamCountdownReminder(db, messaging) {
  ...
  let sent = 0;
  for (const doc of snap.docs) {
    const { exams=[], fcmTokens=[] } = doc.data();
    ...
    for (const exam of exams) {
      ...
      if (![0,1,2,3,4,5,6,7,14,30,60,90].includes(daysLeft)) continue;
      let title, body;
      if (daysLeft === 0) { title=`${exam.name} is today!`; body="Good luck - you've got this."; }
      else if (daysLeft === 1) { title=`${exam.name} is tomorrow`; body="Last chance to review. You're ready."; }
      else if (daysLeft <= 7) { title=`${daysLeft} days to ${exam.name}`; body="The final stretch. Stay focused."; }
      else { title=`${daysLeft} days to ${exam.name}`; body=`Keep going. ${exam.name} on ${new Date(exam.date).toLocaleDateString("en-IN",{day:"numeric",month:"short"})}.`; }
      sent += await sendToUser(db, messaging, doc.id, title, body,
        { url:"/", action:"open_exams", examId: exam.id || exam.name }
      );
    }
  }
  return sent;
...
async function main() {
  ...
  if (!["ttly","weekly","exam"].includes(job))
  ...
  let sent = 0;
  if (job === "ttly") sent = await runTtlyNudge(db, messaging);
  if (job === "weekly") sent = await runWeeklyRatingNudge(db, messaging);
  if (job === "exam") sent = await runExamCountdownReminder(db, messaging);
  console.log(`[${job}] Notifications sent: ${sent}`);
}
